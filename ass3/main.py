...
##################################
    ...
    while True:
        ...
        if(inMsg.type == '0'):
            serverOut = "{}: Login attempt by: {}".format(datetime.now().strftime('%b %d %Y %H:%M:%S'),repr(address))
            logFile.write(serverOut+'\n')
            logFile.flush()
            print(serverOut)
            name, pw = inMsg.text.split(':',1)
            attempt = userDB.loginUser(name, pw, address)
            if(attempt == 0):
                serverOut = "{}: {} logged in. Address:  {}".format(datetime.now().strftime('%b %d %Y %H:%M:%S'), name, repr(address))
                logFile.write(serverOut+'\n')
                logFile.flush()
                print(serverOut)
            ...
            serverOut = "{}: {}: {}".format(datetime.now().strftime('%b %d %Y %H:%M:%S'), user, inMsg.text)
            logFile.write(serverOut+'\n')
            logFile.flush()
            print(serverOut)
